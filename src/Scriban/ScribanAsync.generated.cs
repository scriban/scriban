        ...
        private async ValueTask<ScriptExpression> RewriteAsync(TemplateContext context, int precedence)
        {
            ...
            while (_index < Count)
            {
                ...
                if (nextExpression is ScriptArgumentBinary bin)
                {
                    ...
                    {Left = leftValue, Operator = bin.Operator, Right = rightValue, };
                    continue;
                }

                if (nextExpression is IScriptVariablePath)
                {
                    var result = await context.EvaluateAsync(nextExpression, true).ConfigureAwait(false);
                    ...
                    if (result is IScriptCustomFunction function)
                    {
                        ...
                        if (maxArg == 1 || function.IsExpressionParameter(0))
                        {
                            if (PrecedenceTopLevel == precedence || leftValue == null)
                            {
                                ...
                                {
                                    leftValue = new ScriptBinaryExpression()
                                    {Left = leftValue, Operator = ScriptBinaryOperator.Multiply, Right = functionCall, };
                                }
...

                {
                    ...
                    else
                    {
                        ...
                        var rightValue = await RewriteAsync(context, PrecedenceOfMultiply).ConfigureAwait(false);
                        ...
                        leftValue = new ScriptBinaryExpression()
                        {Left = leftValue, Operator = ScriptBinaryOperator.Multiply, Right = rightValue, };
                    }
                }
            }
    ...
    public partial class ScriptFunctionCall
    {
        ...
        public override async ValueTask<object> EvaluateAsync(TemplateContext context)
        {
            if (context.UseScientific && !DirectCall)
            {
                ...
                var expression = await rewrite.RewriteAsync(context).ConfigureAwait(false);

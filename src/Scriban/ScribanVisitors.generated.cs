...
namespace Scriban.Syntax
{
    ...
    public abstract partial class ScriptRewriter
    {
        ...
        {
            var newDoToken = (ScriptToken)Visit((ScriptNode)node.DoToken);
            var newFunction = (ScriptFunction)Visit((ScriptNode)node.Function);
            return new ScriptAnonymousFunction() { DoToken = newDoToken, Function = newFunction };
        }
        ...
        public override ScriptNode Visit(ScriptFunction node)
        {
            ...
            return new ScriptFunction() { Name = newName, OpenParen = newOpenParen, Parameters = newParameters, CloseParen = newCloseParen, EqualToken = newEqualToken, Body = newBody, IsAnonymous = node.IsAnonymous };
        }

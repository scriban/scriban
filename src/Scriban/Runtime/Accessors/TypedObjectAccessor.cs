...
namespace Scriban.Runtime.Accessors
{
    ...
    class TypedObjectAccessor : IObjectAccessor
    {
        ...
        {
            value = null;
            if (_members.TryGetValue(member, out var memberAccessor))
            {
                var fieldAccessor = memberAccessor as FieldInfo;
                if (fieldAccessor is not null)
                {
        ...
        public bool HasIndexer => _indexer is not null;

        public bool TrySetValue(TemplateContext context, SourceSpan span, object target, string member, object? value)
        {
            if (!_members.TryGetValue(member, out var memberAccessor) || memberAccessor is null)
                return false;
        ...
        private void PrepareMembers()
        {
            ...
            while (type is not null)
            {
                foreach (var field in type.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly))
                {
                    ...
                    if (keep && !field.IsStatic && field.IsPublic && !field.IsLiteral && (_filter is null || _filter(field)))
                    {
                ...
                foreach (var property in type.GetProperties(BindingFlags.Instance | BindingFlags.Public | BindingFlags.DeclaredOnly))
                {
                    ...
                    if (keep && property.CanRead && getMethod is not null && !getMethod.IsStatic && getMethod.IsPublic && (_filter is null || _filter(property)))
                    {
